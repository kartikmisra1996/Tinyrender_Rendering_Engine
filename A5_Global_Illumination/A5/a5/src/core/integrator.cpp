...
size_t Integrator::selectEmitter(float sample, float& pdf) const {
    ...
    id = min(id, scene.emitters.size() - 1);
...
void Integrator::sampleEmitterDirection(Sampler& sampler,
                                        ...
                                        v3f& d,
                                        float& pdf) const {
    const v3f local = Warp::squareToCosineHemisphere(sampler.next2D());
    d = Frame(n).toWorld(local);
    pdf = Warp::squareToCosineHemispherePdf(local);
}
