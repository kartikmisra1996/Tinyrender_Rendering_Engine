...
struct DirectIntegrator : Integrator {
	...
	void sampleSphereByArea(const p2f& sample,
		...
		float& pdf) const {
		// TODO: Implement this
		v3f sampleOnSphere = emitterRadius * Warp::squareToUniformSphere(sample);


		v3f pointWorld = sampleOnSphere + emitterCenter;
		pos = pointWorld;
		ne = glm::normalize(sampleOnSphere);
		wiW = glm::normalize(pointWorld - pShading);
