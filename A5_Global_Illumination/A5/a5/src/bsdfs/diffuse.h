...
struct DiffuseBSDF : BSDF {
    ...
    v3f eval(const SurfaceInteraction& i) const override {
		...
		if ((Frame::cosTheta(i.wo) > 0 && Frame::cosTheta(i.wi)) > 0) {
			...
			v3f albEval = albedo->eval(worldData, i);
			v3f diffuse = (albEval / M_PI);

			v3f frame = glm::normalize(i.frameNs.toLocal(i.frameNs.n));
			val = diffuse * (glm::dot(i.wi, frame));
		}
		else {
			val = v3f(0.f);
		}
    ...
    v3f sample(SurfaceInteraction& i, const v2f& sample, float* pdf1) const override {
		v3f val(0.f);
